...
public class ShangpinDaoImpl extends BaseDao implements ShangpinDao {
    ...
    public int addShangpin(Shangpin shangpin) {
        String sql = "insert into shangpin (id,store_id,group,type,name,description,standard,price,num) " +
                "values(DEFAULT,?,?,?,?,?,?,?,?)";
    ...
    public List<Shangpin> findAllShangpinByGroup(String group) {
        String sql = "select * from shangpin where group = ?";
        List<Shangpin> shangpinList = baseQuery(Shangpin.class, sql,group);
        return shangpinList;
    }

    public List<Shangpin> findAllShangpinByStoreId(String storeId) {
        String sql = "select * from shangpin where store_id = ?";
